...
function my_theme_enqueue_styles() {
	...
	// wp_enqueue_style( 'fontawesome', 'https://cdnjs.cloudflare.com/ajax/libs/font-awesome/6.2.0/css/all.css');
...
add_action('wp_ajax_the_filtered_category', 'the_filtered_category');
add_action( 'wp_ajax_nopriv_the_filtered_category',	'the_filtered_category' );

function the_filtered_category() {
	global $wpdb;
	$filters = [
		'city'		=>	0,
		'category'	=>	0,
	...
	if ( $_SERVER['REQUEST_METHOD'] == 'POST' )	$filters = $_POST['filters'];
	...
	$categories = $wpdb->get_results("SELECT
										...
										FROM
											wp_stylebible_categories 
										WHERE
											category_id IN ( SELECT cat_id FROM wp_stylebible_match_list
														" . (!empty($filters['city']) ? "WHERE city_id = " . $filters['city'] : "") .
														" GROUP BY cat_id) 
										ORDER BY
											order_number");
...
function the_list() {
	...
	$query	=	"SELECT
					...
					wp_stylebible_cities.currency_unit,
		...
		<div class="items">
			...
			<div class="item">
				<div class="img-wrapper">
					<img
						 src="<?php echo generate_img_url($item); ?>"
				...
				<div class="info">
					<?php
					$sub_category = explode('and', $item->sub_cat_name);
					...
					<div class="site">
						<div class="site-left">
							...
							<div class="price">
								<?php
									echo $item->currency_unit . $item->price;
								?>
							</div>
							<?php 
							}
						...
						<div class="site-right">
							<div class="instagram-link">
								<a href="<?php echo $item->instagram_url; ?>" target="_blank" title="<?php echo retrieve_url($item->instagram_url); ?>">
...
function generate_img_url($item) {
	$sub_category_name = explode('and', $item->sub_cat_name);
	$str = $item->city_name . '_';
	foreach($sub_category_name as $row)
		$str .= trim($row) . '_';
	$str .= $item->establishment_name;
	$establishment_img = str_replace(' ', '', $str);
	return esc_url(home_url('/wp-content/uploads/' . $establishment_img . '.jpg' ));
}
